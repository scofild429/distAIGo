...
type Model struct {
	trainGraph *gorgonia.ExprGraph
	evalGraph  *gorgonia.ExprGraph
...
func (m *Model) Predictor(module godl.Module, opts PredictOpts) (Predictor, error) {
	opts.setDefaults()
...
func (m Model) CheckArity(lt godl.LayerType, nodes []*gorgonia.Node, arity int) error {
	if len(nodes) != arity {
		return godl.ErrorF(lt, "arity doesn't match, expected %d, got %d", arity, len(nodes))
	}
