...
func Conv2d(m *Model, opts Conv2dOpts) *Conv2dModule {
	opts.setDefaults()
	lt := godl.AddLayer("PreConv2d")

	w := m.AddWeights(
		lt,
		tensor.Shape{opts.OutputDimension, opts.InputDimension, opts.KernelSize[0], opts.KernelSize[0]},
		NewWeightsOpts{
	...
	fmt.Println("Test the WeightsName for Cond2D", opts.WeightsName)
	var bias *gorgonia.Node
	if opts.WithBias {
		bias = m.AddBias(
			lt,
			tensor.Shape{1, opts.OutputDimension, 1, 1},
			NewWeightsOpts{
