...
func (t *Model) AddWeights(lt godl.LayerType, shape tensor.Shape, opts NewWeightsOpts) *gorgonia.Node {
	return t.AddLearnable(lt, "weight", shape, opts)
}

func (t *Model) AddBias(lt godl.LayerType, shape tensor.Shape, opts NewWeightsOpts) *gorgonia.Node {
	return t.AddLearnable(lt, "bias", shape, opts)
}

func (t *Model) AddLearnable(lt godl.LayerType, typ string, shape tensor.Shape, opts NewWeightsOpts) *gorgonia.Node {
	if opts.UniqueName == "" {
