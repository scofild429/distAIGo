...
type TrainOpts struct {
	...
	CostObserver       func(epoch int, totalEpoch, batch int, totalBatch int, cost float32, rank int)
...
func Train(m *Model, module godl.Module, trainX, trainY, validateX, validateY tensor.Tensor, rank int, opts TrainOpts) error {
	opts.setDefaults()
	...
	for i := 0; i < opts.Epochs; i++ {
		for dl.HasNext() {
			...
			// 	val := item.Value().Data().([]float32)[0]
			// 	fmt.Println("The ", index, "weight has the first element of", val)
			// }
			...
			if opts.CostObserver != nil {
				opts.CostObserver(i, opts.Epochs, dl.CurrentBatch, dl.Batches, costVal.Data().(float32), rank)
			} else {
		...
		// }
		// fmt.Printf("The learnalbes value has totally length of %v is %v \n", len(weights), indexs)
	}
